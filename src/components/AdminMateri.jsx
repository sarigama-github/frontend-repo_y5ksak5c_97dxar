...
export default function AdminMateri() {
  ...
  const saveNew = async (e) => {
    ...
    await fetch(`${BASE}/materials`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(form) })
  ...
  const startEdit = (it) => {
    setEditing(it.id)
    setForm({ title: it.title, content: it.content, category: it.category || '' })
  }

  const saveEdit = async (e) => {
    e.preventDefault()
    if (!editing) return
    await fetch(`${BASE}/materials/${editing}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(form) })
    setEditing(null)
    setForm({ title: '', content: '', category: '' })
    load()
  }
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-8">
      ...
      <form onSubmit={editing ? saveEdit : saveNew} className="bg-white border rounded p-4 mb-6 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">{editing ? 'Simpan Perubahan' : 'Tambah Materi'}</button>
          {editing && <button type="button" onClick={()=>{ setEditing(null); setForm({ title:'', content:'', category:'' }) }} className="text-slate-600">Batal</button>}
        </div>
      </form>
          ...
          {items.map(it => (
            <article key={it.id} className="bg-white border rounded p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <button onClick={()=>startEdit(it)} className="text-blue-600 text-sm">Edit</button>
              </div>
